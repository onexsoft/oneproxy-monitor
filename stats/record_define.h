...
namespace stats{
	...
	typedef struct _client_query_info_t{
		...
		void add_sqlList(unsigned int sqlHashCode);
		void add_failSqlList(unsigned int sqlHashCode);
	} ClientQueryInfo;
	...
	typedef struct sql_info_t{
		...
		void add_tableSet(std::string tableName);
		void add_clientSet(unsigned int clientHashCode);
