...
static inline void le32enc(void *p, uint32_t x)
{
	...
	memcpy(p, (const void*)&tmp, (size_t)sizeof(tmp));
}
 ...
 * Public API.
 */
void MD5Method::md5_reset(struct md5_ctx *ctx)
{
	ctx->nbytes = 0;
	ctx->a = 0x67452301;
	ctx->b = 0xefcdab89;
...
void MD5Method::md5_update(struct md5_ctx *ctx, const void *data, unsigned int len)
{
	unsigned int n;
...
void MD5Method::md5_final(uint8_t *dst, struct md5_ctx *ctx)
{
